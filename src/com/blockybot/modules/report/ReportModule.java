package com.blockybot.modules.report;
...
/**
 * Module for BlockyBot detecting when the report reaction as added to a message
 * then handling it
 * 
 * @author BlockBa5her
 *
 */
public class ReportModule extends BlockyBotModule {
    ...
    private void onReport(Message msg, MessageReaction reac) {
        ...
        for (User usr : users) { // for every user in user reactions
            ...
            BlockyBotEmbed dmEmb = new BlockyBotEmbed(); // create new embed
        ...
        for (Member mod : mods) {
            // new embed
            BlockyBotEmbed modEmb = new BlockyBotEmbed();
