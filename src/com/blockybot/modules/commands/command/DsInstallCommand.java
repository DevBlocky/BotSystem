 ...
 *
 */
public class DsInstallCommand extends BotCommand {
     ...
     * 
     * @param cmd
     *            The command to be invoked with
     */
    public DsInstallCommand(String cmd) {
        super(cmd);
    ...
    private void setResponses() {
        ...
        responses.put("1", new MessageEmbed.Field[] { new MessageEmbed.Field("Depreciated Version",
        ...
        responses.put("3", new MessageEmbed.Field[] {
                new MessageEmbed.Field(
                        "Step 1", "No download available, get from the channel <#403769028793073665>", false),
                new MessageEmbed.Field("Step 2", "Unpack the .zip file using 7zip or WinRaR", false),
                new MessageEmbed.Field("Step 3",
                ...
                new MessageEmbed.Field("Step 6",
                        "***This and onward are optional.***\nPort forward port `33333` to use Terminal", false),
                new MessageEmbed.Field("Step 7",
                        "Change the `settings.ini` file inside of the `Terminal` folder, then open the `Terminal.exe` to run the terminal",
                        false) });
    }
    ...
    public void onInvoke(Message m, String[] args) {
        ...
        if (args.length == 0) {
            ...
            emb.setDescription("" + "You need to include a version as an argument in this command.\n\n" + "Example: `"
                    + parent.getBot().getModuleConfig(BotCommandsModule.class).getString("header") + "installhelp 2.2`");
        ...
        emb.setDescription("" + "You inputted a version that doesn't exist!\n\n" + "Example: `"
                + parent.getBot().getModuleConfig(BotCommandsModule.class).getString("header") + "installhelp 2.2`");
        m.getChannel().sendMessage(emb.build()).queue();
    }
