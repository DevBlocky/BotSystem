package com.blockybot.modules.permissions;

import com.blockybot.core.BlockyBotModule;
import com.blockybot.extensions.Pair;
...
/**
 * Module used for generic permissions without BlockyBot
 * 
 * @author BlockBa5her
 *
 */
public class PermissionsModule extends BlockyBotModule {
    ...
    public void onStart() {
        ...
        JSONArray permSettings = this.config.getJSONArray("roles");
        for (int i = 0; i < permSettings.length(); i++) {
        	JSONObject perm = permSettings.getJSONObject(i);
        	perms.add(new InternalPermission(perm.getString("name"), perm.getString("id")));
        }
    }
     ...
     * 
     * @param m
     *            The member to search permissions for
    ...
    /**
     * Finds the distance from the first perm to the second, negative means that perm2 is below perm1
     * @return The distance between the 2 permission's index
     */
    public int getPermDist(String perm1, String perm2) {
    	return indexOfName(perm1) - indexOfName(perm2);
    }
    
    public int getPermDistForMembers(Member m1, Member m2) {
    	return getPermDist(getMemberHighestPerm(m1).getValue(), getMemberHighestPerm(m2).getValue());
    }
     ...
     * 
     * @param permName
     *            The permission name to check to
     * @param m
     *            The member to check against
    ...
    public void addPermission(String permName, String roleId, String before) {
        ...
        if (before != null) { // if there is a before
            ...
            if (index == -1) // if not found
                throw new RuntimeException(new ArrayStoreException("'before' doesn't exist in array")); // throw
                                                                                                        // exception
        }
