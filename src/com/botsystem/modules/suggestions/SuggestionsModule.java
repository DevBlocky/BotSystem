...
public class SuggestionsModule extends BotSystemModule {
	...
	private static class SuggestionEmbed {
		 ...
		 * @param suggester The suggester in the suggestion
		 */
		 ...
		 * @param field The field to add in the Embed
		...
		/**
		 * Builds the embed
		 * Also adds required suggestion syntax
		 * @return
		 */
		public MessageEmbed build() {
			emb
					.setAuthor(suggester.getName(), null, suggester.getAvatarUrl()) // setting author name and pic url
	                .setTimestamp(Instant.now()) // setting timestamp to now
	                .setFooter("Voice your opinion using the reactions below", null) // Adding footer to alert users of what to do
	 ...
	 * @param m The message of the suggestion
	 */
	private void makeSuggestion(Message m) {
		...
		emb.addField(new MessageEmbed.Field("Suggestion", msgContent, false)); // adding the suggestion field with information
