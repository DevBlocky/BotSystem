...
public class MonitorCommand extends BotCommand {
	...
	public void onInvoke(Message m, String[] args) {		
		...
		emb.addField(new MessageEmbed.Field(
				"Process CPU Load",
				info.get("processCpuStr") + "%",
				true));
		emb.addField(new MessageEmbed.Field(
				"System CPU Load",
				info.get("systemCpuStr") + "%",
				true));
		emb.addField(new MessageEmbed.Field(
				"Process Heap Memory Allocation",
				info.get("heapAllocStr") + "MB / " + info.get("totalMemoryStr") + "MB (" + info.get("heapAllocPercentStr") + "%)",
				true));
		emb.addField(new MessageEmbed.Field(
				"Process Stack Memory Allocation",
				info.get("stackAllocStr") + "MB / " + info.get("totalMemoryStr") + "MB (" + info.get("stackAllocPercentStr") + "%)",
				true));
		emb.addField(new MessageEmbed.Field(
				"Total Process Memory Allocation",
				info.get("totalAllocStr") + "MB / " + info.get("totalMemoryStr") + "MB (" + info.get("totalAllocPercentStr") + "%)",
				true));
		emb.addField(new MessageEmbed.Field(
				"System Memory Usage",
				info.get("systemMemoryStr") + "MB / " + info.get("totalMemoryStr") + "MB (" + info.get("systemMemoryPercentStr") + "%)",
				true));
